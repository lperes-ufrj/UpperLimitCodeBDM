...
def ReadNuclearModelSamples(nu_model):
    if nu_model[1] != '00a':
        fatm = ROOT.TFile.Open(path+'ntuples_grid_newVertex/'+nu_model[1]+'_newVertex/atm_'+nu_model[0]+'.root')
    else:
        fatm = ROOT.TFile.Open(path+'ntuples_grid_newVertex/'+nu_model[1]+'_newVertex/atm_'+nu_model[0]+'_4ana.root')
    fbdm_m05_b1p1 = ROOT.TFile.Open(path+'ntuples_grid_newVertex/'+nu_model[1]+'_newVertex/bdm_m05_b1p1_'+nu_model[1]+'.root') 
    fbdm_m10_b1p1 = ROOT.TFile.Open(path+'ntuples_grid_newVertex/'+nu_model[1]+'_newVertex/bdm_m10_b1p1_'+nu_model[1]+'.root')
    fbdm_m20_b1p1 = ROOT.TFile.Open(path+'ntuples_grid_newVertex/'+nu_model[1]+'_newVertex/bdm_m20_b1p1_'+nu_model[1]+'.root')  
    fbdm_m40_b1p1 = ROOT.TFile.Open(path+'ntuples_grid_newVertex/'+nu_model[1]+'_newVertex/bdm_m40_b1p1_'+nu_model[1]+'.root')
    fbdm_m05_b1p5 = ROOT.TFile.Open(path+'ntuples_grid_newVertex/'+nu_model[1]+'_newVertex/bdm_m05_b1p5_'+nu_model[1]+'.root')  
    fbdm_m10_b1p5 = ROOT.TFile.Open(path+'ntuples_grid_newVertex/'+nu_model[1]+'_newVertex/bdm_m10_b1p5_'+nu_model[1]+'.root')
    fbdm_m20_b1p5 = ROOT.TFile.Open(path+'ntuples_grid_newVertex/'+nu_model[1]+'_newVertex/bdm_m20_b1p5_'+nu_model[1]+'.root')
    fbdm_m40_b1p5 = ROOT.TFile.Open(path+'ntuples_grid_newVertex/'+nu_model[1]+'_newVertex/bdm_m40_b1p5_'+nu_model[1]+'.root')
    fbdm_m05_b10 = ROOT.TFile.Open(path+'ntuples_grid_newVertex/'+nu_model[1]+'_newVertex/bdm_m05_b10_'+nu_model[1]+'.root')     
    fbdm_m10_b10 = ROOT.TFile.Open(path+'ntuples_grid_newVertex/'+nu_model[1]+'_newVertex/bdm_m10_b10_'+nu_model[1]+'.root')
    if nu_model[1] != '01a':
        fbdm_m20_b10 = ROOT.TFile.Open(path+'ntuples_grid_newVertex/'+nu_model[1]+'_newVertex/bdm_m20_b10_'+nu_model[1]+'.root')
    fbdm_m40_b10 = ROOT.TFile.Open(path+'ntuples_grid_newVertex/'+nu_model[1]+'_newVertex/bdm_m40_b10_'+nu_model[1]+'.root')
...
def GetBDTResponse(tree):
    ...
    reader.BookMVA("TMVAClassification","/home/leoperes/Dropbox/BoostedDM/Final_Analysis/UpperLimitCode/BDT/dataset/weights/TMVAClassification_BDTAtmCCNC_202503120817.weights.xml")
...
Eff_Bkg_default = np.loadtxt('../Optimization_DefaultNuclearModel_final/Eff_Bkg_index_00a.txt', delimiter=' ', usecols=[1,2,6,7], skiprows=1)
...
for nc_id, nuclearmodel in enumerate(list_nuclearmodels):
    ...
    for index, sig_tree in enumerate(intrees_reco[1:]):
        ...
        if nuclearmodel[1] == '01a' and index == 10: 
            #print(nuclearmodel[1])
            CVNScore = Eff_Bkg_default[index+1,2]
            BDT_Score = Eff_Bkg_default[index+1,3]
            CosCutLeft = Eff_Bkg_default[index+1,0]
            CosCutRight = Eff_Bkg_default[index+1,1]
            nSignalSimulatedEvts = simu_entries[index+1]
            #print(CVNScore,BDT_Score,CosCutLeft,CosCutRight,nSignalSimulatedEvts)
