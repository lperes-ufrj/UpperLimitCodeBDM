...
parser = argparse.ArgumentParser(description="Calculate 100 kt-year CLs upper limits with systematics.")
parser.add_argument(
    "--seed",
    default=12345,
    type=int,
...
parser.add_argument(
    ...
    required=True,
    type=Path,
    help="Path to the 100 kt-year NuclearModelsOptSelectionApplication_final/generated/<timestamp> directory.",
)
parser.add_argument("--sample-start", default=0, type=int, help="First sample index to process.")
parser.add_argument("--sample-stop", default=12, type=int, help="Stop sample index, Python-slice style.")
parser.add_argument(
    "--poi-upper-scale",
    default=1.6,
    type=float,
    help="Scale the upper end of each gZ'^4 scan range. 100 kt-year limits are weaker than 400 kt-year.",
)
...
# Number of target Argon nuclei and livetime for the low-exposure analysis.
# This is 100 kt-year, implemented as 10 kton x 10 years.
NA_dune = 1 * 1.5e32             # 10 kton
...
path_eff = str(args.path_eff) + "/" #Path to the folder with the optimal cuts for each nuclear model and BDM sample
...
poi = [np.linspace(p[0], p[-1] * args.poi_upper_scale, len(p), dtype=np.float64) for p in poi]
...
##print(str(path))
for i in range(args.sample_start,args.sample_stop): #Each BDM sample gamma and mass value 
    ...
    Sensitivity_Info.write('EXPOSURE: 100 kt-year\n')
    Sensitivity_Info.write('NA_dune: '+str(NA_dune)+'\n')
    Sensitivity_Info.write('livetime_dune: '+str(livetime_dune)+'\n')
    ...
    positive_eff_mask = eff_syst > 0
    B_syst = B_syst[positive_eff_mask]
    NA_dune_syst = NA_dune_syst[positive_eff_mask] # Physical cut, only positive efficiency throws.
    eff_syst = eff_syst[positive_eff_mask] 
